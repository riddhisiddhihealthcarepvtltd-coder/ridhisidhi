...
const Footer = ({ onOpenAppointment }) => {
  return (
    <footer className="bg-[#0d3033] text-[#b8cfd1] relative overflow-hidden pt-14 pb-8 border-t border-white/10">
      <div className="container mx-auto max-w-[1400px] px-8 lg:px-16 xl:px-24 relative z-10">
        
        {/* Top Info Banner Box */}
        <div className="bg-[#123f43] rounded-2xl p-6 mb-12 border border-white/10 flex flex-col md:flex-row items-center justify-between gap-6">
          <div className="flex items-center gap-4">
            <div className="w-12 h-12 rounded-full bg-white/10 flex items-center justify-center shrink-0">
              <Calendar className="w-6 h-6 text-[#1f8a86]" />
            </div>
            <div>
          ...
          <button
            ...
            className="px-6 py-3 rounded-full bg-[#1f8a86] hover:bg-[#12635f] text-white font-heading font-bold text-xs uppercase tracking-wider transition-colors duration-300 shrink-0 shadow-md"
          >
            Book Appointment
          </button>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8 pb-10 border-b border-white/10">
          ...
          <div className="space-y-4">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-[11px] text-[#1f8a86] font-bold">Gokarneshwor-5, Kathmandu</p>
              </div>
            </div>
            ...
            <div className="space-y-1.5 text-xs">
              <div className="flex items-center gap-2">
                <FileText className="w-3.5 h-3.5 text-[#1f8a86]" />
                <span className="font-semibold text-white">PAN:</span> 600624763
              </div>
              <div className="flex items-center gap-2">
                <ShieldCheck className="w-3.5 h-3.5 text-[#1f8a86]" />
          ...
          {/* Col 2: Services */}
          <div>
            <h4 className="font-heading font-bold text-white text-xs uppercase tracking-wider mb-4 border-l-2 border-[#1f8a86] pl-2">
              Our Services
            </h4>
            <ul className="space-y-2 text-xs">
              {servicesData.map((svc) => (
                <li key={svc.id}>
                  <Link to="/services" className="hover:text-[#1f8a86] transition-colors">
                    {svc.title}
                  </Link>
                </li>
              ))}
          ...
          <div>
            ...
            <ul className="space-y-2 text-xs">
              {navLinks.map((link) => (
                <li key={link.name}>
                  <Link to={link.path} className="hover:text-[#1f8a86] transition-colors">
                    {link.name}
                  </Link>
                </li>
              ))}
